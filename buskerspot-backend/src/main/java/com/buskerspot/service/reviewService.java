...
public class ReviewService {
    ...
    @Transactional
    public void createReview(Long userId, Map<String, Object> request) {
        Object perfIdObj = request.get("performance_id");
        Object ratingObj = request.get("rating");
        String comment = (String) request.get("comment");

        if (perfIdObj == null || ratingObj == null || comment == null || comment.trim().isEmpty()) {
            throw new CustomException("필수 데이터가 누락되었습니다.", HttpStatus.BAD_REQUEST);
        }

        Long performanceId = Long.valueOf(perfIdObj.toString());
        ...
        // 리뷰 저장 (Entity에 user_name 컬럼이 있다고 가정, 또는 쿼리 직접 수행)
